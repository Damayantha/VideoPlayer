...
import com.cyberkey.commons.views.MySearchMenu
import com.cyberkey.videoplayer.R
import com.cyberkey.videoplayer.adapters.DirectoryAdapter
import com.cyberkey.videoplayer.databinding.DialogDirectoryPickerBinding
...
class PickDirectoryDialog(
    ...
    init {
        ...
        builder.apply {
            activity.setupDialogStuff(binding.root, this, com.cyberkey.commons.R.string.select_destination) { alertDialog ->
                dialog = alertDialog
    ...
    private fun configureSearchView() = with(searchView) {
        updateHintText(context.getString(com.cyberkey.commons.R.string.search_folders))
    ...
    private fun MySearchMenu.setSearchViewListeners() {
        onSearchOpenListener = {
            updateSearchViewLeftIcon(com.cyberkey.commons.R.drawable.ic_cross_vector)
        }

        onSearchClosedListener = {
            searchEditText.clearFocus()
            activity.hideKeyboard(searchEditText)
            updateSearchViewLeftIcon(com.cyberkey.commons.R.drawable.ic_search_vector)
        }
    ...
    private fun checkPlaceholderVisibility(dirs: ArrayList<Directory>) = with(binding) {
        ...
        if (folderSearchView.isSearchOpen) {
            directoriesEmptyPlaceholder.text = root.context.getString(com.cyberkey.commons.R.string.no_items_found)
        }
        ...
        val adapter = DirectoryAdapter(activity, dirs.clone() as ArrayList<Directory>, null, binding.directoriesGrid, true) {
            ...
            if (clickedDir.subfoldersCount == 1 || !activity.config.groupDirectSubfolders) {
                if (isPickingCopyMoveDestination && path.trimEnd('/') == sourcePath) {
                    activity.toast(com.cyberkey.commons.R.string.source_and_destination_same)
                    return@DirectoryAdapter
                } else if (isPickingCopyMoveDestination && activity.isRestrictedWithSAFSdk30(path) && !activity.isInDownloadDir(path)) {
                    activity.toast(com.cyberkey.commons.R.string.system_folder_copy_restriction, Toast.LENGTH_LONG)
                    return@DirectoryAdapter
                } else {
