...
import com.cyberkey.videoplayer.interfaces.DirectoryOperationsListener
import com.cyberkey.videoplayer.jobs.NewPhotoFetcher
...
class MainActivity : SimpleActivity(), DirectoryOperationsListener, LevelPlayInterstitialListener, ImpressionDataListener {
    ...
    private var mLastMediaHandler = Handler(Looper.myLooper()!!)
    private var mTempShowHiddenHandler = Handler(Looper.myLooper()!!)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (config.hideCreateNewFolder){
            binding.createNewFolder.visibility=View.GONE
        }

        ensureBackgroundThread { binding.progressCircular.show() }
        ...
        if (BuildConfig.APPLICATION_ID != "com.cyberkey.videoplayer.pro" || !BuildConfig.APPLICATION_ID.contains(".pro")) {
        ...
        binding.directoriesGrid.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                if (dy > 0) {
                    binding.createNewFolder.shrink()
                } else {
                    binding.createNewFolder.extend()
                }
...
/*    private fun createAndloadBanner() {
        ...
        mIronSourceBannerLayout?.let {
            ...
            val bannerFooter = findViewById<FrameLayout>(R.id.banner_footer)

            bannerFooter.addView(it, 0, layoutParams)

            // set the banner listener
            it.levelPlayBannerListener = object : LevelPlayBannerListener {
                override fun onAdLoaded(p0: AdInfo?) {
                    bannerFooter.visibility = View.VISIBLE
                }
            ...
            //Toast.makeText(this@AdsActivity, "IronSource.createBanner returned null", Toast.LENGTH_LONG).show()
    ...
    fun popupSnackbarForCompleteUpdate() {
        Snackbar.make(
            ...
            setActionTextColor(resources.getColor(com.cyberkey.commons.R.color.color_primary))
            show()
        }
    }
 ...
 /*   private fun popupSnackbarForCompleteUpdate() {
        Snackbar.make(
            ...
            setActionTextColor(ContextCompat.getColor(this@MainActivity, com.cyberkey.commons.R.color.color_primary))
            show()
        }
    }*/
    ...
    override fun onResume() {
        ...
        if (config.searchAllFilesByDefault) {
            binding.mainMenu.updateHintText(getString(com.cyberkey.commons.R.string.search_files))
        } else {
            binding.mainMenu.updateHintText(getString(com.cyberkey.commons.R.string.search_folders))
        }
    }
    ...
    private fun refreshMenuItems() {
        if (!mIsThirdPartyIntent) {
            binding.mainMenu.getToolbar().menu.apply {
                ...
                findItem(R.id.more_apps_from_us).isVisible = !resources.getBoolean(com.cyberkey.commons.R.bool.hide_google_relations)
            }
        }
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mainMenu.onSearchTextChangedListener = { text ->
            setupAdapter(mDirsIgnoringSearch, text.trim())
        ...
        if (config.tempFolderPath.isNotEmpty()) {
            ...
            if (getDoesFilePathExist(newFolder.absolutePath) && newFolder.isDirectory) {
                if (newFolder.getProperSize(true) == 0L && newFolder.getFileCount(true) == 0 && newFolder.list()?.isEmpty() == true) {
                    toast(String.format(getString(com.cyberkey.commons.R.string.deleting_folder), config.tempFolderPath), Toast.LENGTH_LONG)
    ...
    override fun deleteFolders(folders: ArrayList<File>) {
        ...
        when {
            ...
            fileDirItems.size == 1 -> {
                try {
                    toast(String.format(getString(com.cyberkey.commons.R.string.deleting_folder), fileDirItems.first().name))
                } catch (e: Exception) {
                    showErrorToast(e)
                }
            }
                ...
                val baseString =
                    if (config.useRecycleBin && !config.tempSkipRecycleBin) com.cyberkey.commons.R.plurals.moving_items_into_bin else com.cyberkey.commons.R.plurals.delete_items
    ...
    private fun changeColumnCount() {
        ...
        for (i in 1..MAX_COLUMN_COUNT) {
            items.add(RadioItem(i, resources.getQuantityString(com.cyberkey.commons.R.plurals.column_counts, i, i)))
        }
            ...
            try {
                if (mediaDB.getDeletedMediaCount() > 0) {
                    val recycleBin = Directory().apply {
                        path = RECYCLE_BIN
                        name = getString(com.cyberkey.commons.R.string.recycle_bin)
        ...
        if (dirs.map { it.path }.contains(FAVORITES)) {
            if (mediaDB.getFavoritesCount() > 0) {
                val favorites = Directory().apply {
                    path = FAVORITES
                    name = getString(com.cyberkey.commons.R.string.favorites)
                    location = LOCATION_INTERNAL
                }
