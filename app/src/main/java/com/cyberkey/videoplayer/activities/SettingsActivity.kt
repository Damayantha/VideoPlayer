...
import com.cyberkey.commons.dialogs.*
import com.cyberkey.commons.extensions.*
import com.cyberkey.commons.helpers.*
import com.cyberkey.commons.models.RadioItem
import com.cyberkey.videoplayer.R
import com.cyberkey.videoplayer.databinding.ActivitySettingsBinding
import com.cyberkey.videoplayer.dialogs.*
import com.cyberkey.videoplayer.extensions.*
import com.cyberkey.videoplayer.helpers.*
import com.cyberkey.videoplayer.models.AlbumCover
import java.io.File
...
class SettingsActivity : SimpleActivity() {
    ...
    private fun setupSettingItems() {
        ...
        arrayOf(
            ...
            binding.additionalSettings
        ).forEach {
            it.setTextColor(getProperPrimaryColor())
        }
    }

    private fun setupAdditionlSettings() {
        val navigationMode = getNavigationBarInteractionMode(this)
        when (navigationMode) {
            NAVIGATION_BAR_INTERACTION_MODE_THREE_BUTTON -> {
                binding.settingsHideHomeUi.visibility= View.VISIBLE
            }
            NAVIGATION_BAR_INTERACTION_MODE_TWO_BUTTON -> {
                binding.settingsHideHomeUi.visibility= View.VISIBLE
            }
            NAVIGATION_BAR_INTERACTION_MODE_GESTURE -> {
                binding.settingsHideHomeUi.visibility= View.GONE
            }
        }
        binding.checkSettingsHideHomeUi.isChecked = config.hideHmeUI
        binding.checkSettingsHideCreateNewFolder.isChecked = config.hideCreateNewFolder
       // binding.checkRemoveSplashScreen.isChecked = cc.newInstance(applicationContext).removeSplashScreen
        if (binding.checkSettingsHideHomeUi.isChecked ){
            binding.checkSettingsHideHomeUi.text="Show navigation icons in home screen (System UI)"
        }
        if (binding.checkSettingsHideCreateNewFolder.isChecked ){
            binding.checkSettingsHideCreateNewFolder.text="Show Create New Folder button"
        }
//        if (binding.checkRemoveSplashScreen.isChecked ){
//            binding.checkRemoveSplashScreen.text="Show Create New Folder button"
//        }
        binding.settingsHideHomeUi.setOnClickListener {
            if (!config.hideHmeUI) {
                binding.checkSettingsHideHomeUi.isChecked = true
                config.hideHmeUI = true
                binding.checkSettingsHideHomeUi.text="Show navigation icons in home screen (System UI)"
            }else{
                binding.checkSettingsHideHomeUi.isChecked = false
                config.hideHmeUI = false
                binding.checkSettingsHideHomeUi.text="Hide navigation icons in home screen (System UI)"
            }
        }
        binding.settingsHideCreateNewFolder.setOnClickListener {
            if (!config.hideCreateNewFolder) {
                binding.checkSettingsHideCreateNewFolder.isChecked = true
                config.hideCreateNewFolder = true
                binding.checkSettingsHideCreateNewFolder.text="Show Create New Folder button"
            }else{
                binding.checkSettingsHideCreateNewFolder.isChecked = false
                config.hideCreateNewFolder = false
                binding.checkSettingsHideCreateNewFolder.text="Hide Create New Folder button"
            }
        }



//        binding.settingsRemoveSplashScreen.setOnClickListener {
//            if (!cc.newInstance(applicationContext).removeSplashScreen) {
//                binding.checkRemoveSplashScreen.isChecked = true
//                cc.newInstance(applicationContext).removeSplashScreen= true
//                binding.checkRemoveSplashScreen.text="Remove splash screen"
//            }else{
//                binding.checkRemoveSplashScreen.isChecked = false
//                cc.newInstance(applicationContext).removeSplashScreen = false
//                binding.checkRemoveSplashScreen.text="Show splash screen"
//            }
//        }
    }


    val NAVIGATION_BAR_INTERACTION_MODE_THREE_BUTTON = 0
    val NAVIGATION_BAR_INTERACTION_MODE_TWO_BUTTON = 1
    val NAVIGATION_BAR_INTERACTION_MODE_GESTURE = 2
    ...
    private fun setupManageIncludedFolders() {
        if (isRPlus() && !isExternalStorageManager()) {
            binding.settingsManageIncludedFolders.text =
                "${getString(R.string.manage_included_folders)} (${getString(com.cyberkey.commons.R.string.no_permission)})"
        } else {
    ...
    private fun setupShowHiddenItems() {
        if (isRPlus() && !isExternalStorageManager()) {
            binding.settingsShowHiddenItems.text =
                "${getString(com.cyberkey.commons.R.string.show_hidden_items)} (${getString(com.cyberkey.commons.R.string.no_permission)})"
        } else {
            binding.settingsShowHiddenItems.setText(com.cyberkey.commons.R.string.show_hidden_items)
        }
                ...
                if (success) {
                    ...
                    if (config.isHiddenPasswordProtectionOn) {
                        val confirmationTextId = if (config.hiddenProtectionType == PROTECTION_FINGERPRINT)
                            com.cyberkey.commons.R.string.fingerprint_setup_successfully else com.cyberkey.commons.R.string.protection_setup_successfully
                        ConfirmationDialog(this, "", confirmationTextId, com.cyberkey.commons.R.string.ok, 0) { }
                    }
                }
            }
        }
    ...
    private fun setupEmptyRecycleBin() {
        ...
        binding.settingsEmptyRecycleBinHolder.setOnClickListener {
            if (mRecycleBinContentSize == 0L) {
                toast(com.cyberkey.commons.R.string.recycle_bin_empty)
            } else {
                ...
                ExportFavoritesDialog(this, getExportFavoritesFilename(), true) { path, filename ->
                    Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
                        ...
                        try {
                            ...
                            toast(com.cyberkey.commons.R.string.system_service_disabled, Toast.LENGTH_LONG)
                        } catch (e: Exception) {
                            showErrorToast(e)
                        }
                    }
    ...
    private fun exportFavoritesTo(outputStream: OutputStream?) {
        ...
        ensureBackgroundThread {
            ...
            if (favoritePaths.isNotEmpty()) {
                ...
                toast(com.cyberkey.commons.R.string.exporting_successful)
            } else {
                toast(com.cyberkey.commons.R.string.no_items_found)
            }
        }
    }

    private fun getExportFavoritesFilename(): String {
        val appName = baseConfig.appId.removeSuffix(".debug").removeSuffix(".pro").removePrefix("com.cyberkey.")
    ...
    private fun importFavorites(inputStream: InputStream?) {
        ...
        ensureBackgroundThread {
            ...
            toast(if (importedItems > 0) com.cyberkey.commons.R.string.importing_successful else com.cyberkey.commons.R.string.no_entries_for_importing)
        }
    }
    ...
    private fun parseFile(inputStream: InputStream?) {
        ...
        toast(if (configValues.size > 0) com.cyberkey.commons.R.string.settings_imported_successfully else com.cyberkey.commons.R.string.no_entries_for_importing)
        runOnUiThread {
            setupSettingItems()
        }
    }
