...
import com.cyberkey.videoplayer.databinding.ActivityMediaBinding
...
class MediaActivity : SimpleActivity(), MediaOperationsListener {
    ...
    private fun searchQueryChanged(text: String) {
        ensureBackgroundThread {
            try {
                ...
                runOnUiThread {
                    if (grouped.isEmpty()) {
                        binding.mediaEmptyTextPlaceholder.text = getString(com.cyberkey.commons.R.string.no_items_found)
    ...
    private fun tryLoadGallery() {
        handlePermission(getPermissionToRequest()) {
            if (it) {
                val dirName = when {
                    mPath == FAVORITES -> getString(com.cyberkey.commons.R.string.favorites)
                    mPath == RECYCLE_BIN -> getString(com.cyberkey.commons.R.string.recycle_bin)
                    mPath == config.OTGPath -> getString(com.cyberkey.commons.R.string.usb)
                    else -> getHumanizedFilename(mPath)
                }

                val searchHint = if (mShowAll) {
                    getString(com.cyberkey.commons.R.string.search_files)
                } else {
                    getString(com.cyberkey.commons.R.string.search_in_placeholder, dirName)
                }
    ...
    private fun startAsyncTask() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val media = GetMediaAsynctask(applicationContext, mPath, mIsGetImageIntent, mIsGetVideoIntent, mShowAll, null).getMedia()
