...
import com.cyberkey.commons.dialogs.RenameDialog
...
import com.cyberkey.videoplayer.databinding.*
...
class MediaAdapter(
    ...
    private fun renameFile() {
        ...
        if (isRPlus() && isSDOrOtgRootFolder && !isExternalStorageManager()) {
            activity.toast(com.cyberkey.commons.R.string.rename_in_sd_card_system_restriction, Toast.LENGTH_LONG)
            finishActMode()
            return
        }
    ...
    private fun handleRotate(paths: List<String>, degrees: Int) {
        ...
        activity.toast(com.cyberkey.commons.R.string.saving)
        ensureBackgroundThread {
            paths.forEach {
    ...
    private fun copyMoveTo(isCopyOperation: Boolean) {
        ...
        if (!isCopyOperation && paths.any { it.startsWith(recycleBinPath) }) {
            activity.toast(com.cyberkey.commons.R.string.moving_recycle_bin_items_disabled, Toast.LENGTH_LONG)
        }
    ...
    private fun askConfirmDelete() {
        ...
        val itemsAndSize = if (itemsCnt == 1) {
            ...
            val deleteItemsString = resources.getQuantityString(com.cyberkey.commons.R.plurals.delete_items, itemsCnt, itemsCnt)
            "$deleteItemsString ($fileSize)"
        }

        val isRecycleBin = firstPath.startsWith(activity.recycleBinPath)
        val baseString =
            if (config.useRecycleBin && !config.tempSkipRecycleBin && !isRecycleBin) com.cyberkey.commons.R.string.move_to_recycle_bin_confirmation else com.cyberkey.commons.R.string.deletion_confirmation
    ...
    private fun setupThumbnail(view: View, medium: Medium) {
        ...
        bindItem(view, medium).apply {
            ...
            if (medium.isVideo()) {
                playPortraitOutline?.setImageResource(com.cyberkey.commons.R.drawable.ic_play_vector)
